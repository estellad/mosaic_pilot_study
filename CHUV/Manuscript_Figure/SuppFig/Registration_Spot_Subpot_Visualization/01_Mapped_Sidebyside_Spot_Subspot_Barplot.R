...
# seu <- readRDS(paste0(savepath.spotclean, paste0("/", str_to_title(disease), "-merge-SCTpostSpotClean.rds")))
# CD <- data.frame(Barcode = colnames(seu), 
#                  Region = seu@meta.data$Region)
# 
# mapped_barcode_order <- mapped_all_spot %>% 
#   filter(Section_ID_Vis == "B1_2") %>%
#   select(barcode) %>% 
#   dplyr::rename(Barcode = barcode) %>%
#   mutate(Barcode = paste0(Barcode, "_1"))
# 
# mapped_barcode_order <- mapped_barcode_order %>%
#   left_join(CD)
# 
# mapped_all_spot$Region <- ifelse(mapped_all_spot$Section_ID_Vis == "B1_2", mapped_barcode_order$Region, mapped_all_spot$Region)
# table(mapped_all_spot$Region)
# 
# to_remove <- paste0(mapped_all_spot$barcode[mapped_all_spot$Section_ID_Vis == "B1_2"], "_1")[which(!(paste0(mapped_all_spot$barcode[mapped_all_spot$Section_ID_Vis == "B1_2"], "_1") %in% colnames(seu)))]
# 
# # [1] "ATTCCTGCCTTAACAG-1_1" "CAATCGTCGGAGCATG-1_1" "CACGTACGTAGCTCAT-1_1" "CCGGATCTCCGAAGCG-1_1" "CTAAGTCAAGATCATG-1_1" "GGTACTGTGATCTTCC-1_1" "GTCTATCCAGGACGTC-1_1"
# # [8] "GTTATTCGGCCATTCG-1_1" "TCAACACCAATCATTA-1_1" "TCAAGTGATGGTTAAT-1_1"
# 
# mapped_all_spot <- mapped_all_spot %>%
#   filter(!(Section_ID_Vis == "B1_2" & barcode %in% substr(to_remove, 1, nchar(to_remove) - 2))) # 10 less spots mapped after B1_2 Patho QC
# 
# 
# ## Subspot
# mapped_subspot_barcode_order <- mapped_all_subspot %>% 
#   filter(Section_ID_Vis == "B1_2") %>%
#   select(barcode2) %>% 
#   dplyr::rename(Barcode = barcode2) %>%
#   mutate(Barcode = paste0(Barcode, "_1"))
# 
# mapped_subspot_barcode_order <- mapped_subspot_barcode_order %>%
#   left_join(CD)
# 
# mapped_all_subspot$Region <- ifelse(mapped_all_subspot$Section_ID_Vis == "B1_2", mapped_subspot_barcode_order$Region, mapped_all_subspot$Region)
# table(mapped_all_subspot$Region)
# 
# to_remove_sub <- paste0(mapped_all_subspot$barcode2[mapped_all_subspot$Section_ID_Vis == "B1_2"], "_1")[which(!(paste0(mapped_all_subspot$barcode2[mapped_all_subspot$Section_ID_Vis == "B1_2"], "_1") %in% colnames(seu)))]
# 
# mapped_all_subspot <- mapped_all_subspot %>%
#   filter(!(Section_ID_Vis == "B1_2" & barcode2 %in% substr(to_remove_sub, 1, nchar(to_remove_sub) - 2))) # 78 less subspots mapped after B1_2 Patho QC
...
p <- ggplot(mapping = aes(y = Count, linetype = Resolution, fill = Indication)) + 
  ...
  geom_text(data = plt_df %>% filter(Resolution == "Spot") %>% filter(Count > 0), aes(x = as.numeric(cell_fraction) + 0.15, color = Indication, label = Count), hjust=1, vjust=-3.5, size = 3.5, position = "stack") + 
  geom_col(data = plt_df %>% filter(Resolution == "Subspot"), aes(x = as.numeric(cell_fraction) - 0.15), width = 0.28, color = "black") +
  geom_text(data = plt_df %>% filter(Resolution == "Subspot") %>% filter(Count > 0), aes(x = as.numeric(cell_fraction) - 0.15, color = Indication, label = Count), hjust=1, vjust=4.5, size = 3.5, position = "stack") + 
  scale_fill_manual(name = "Indication", labels = c("Breast", "Lung"), values = c("#00c78cff", "#ffa54fff")) + 
  scale_color_manual(name = "Indication", labels = c("Breast", "Lung"), values = c("#00c78cff", "#ffa54fff")) + 
...
plot_each_region <- function(mapped_all_plt_subset){
  ggplot(data = mapped_all_plt_subset, aes(x = Region, y = prop, fill = Region, linetype = Resolution, group = Resolution)) +
    geom_bar(stat = "identity", position = position_dodge(width = bar_width), color = "black", width = bar_width) +
