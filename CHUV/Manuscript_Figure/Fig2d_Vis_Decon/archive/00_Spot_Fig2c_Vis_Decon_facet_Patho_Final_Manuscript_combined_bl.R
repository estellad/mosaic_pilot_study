...
decon_patho_gather_breast <- read.csv(file.path(vis_decon_path, "vis_breast_decon_long.csv"))
decon_patho_gather_lung <- read.csv(file.path(vis_decon_path, "vis_lung_decon_long.csv"))
...
stroma_vessel_related <- c("Large_Vessel",
                           "Intratumoral_Stroma",
                           "Intratumoral_Vessel",
                           "Normal_Lung_Epithelium")

breast_lung_patho <- c(tumor_related, lymphocytes_related, macrophage_related, stroma_vessel_related)
facet_name <- c("Tumor_related", "Stroma_Vessel_related", "Lymphocytes_related", "Macrophage_related")
...
decon_patho_gather_breast_subset <- decon_patho_gather_breast %>%
  ...
  mutate(Region = case_when(Region %in% tumor_related ~ "Tumor_related",
                            ...
                            Region %in% stroma_vessel_related ~ "Stroma_Vessel_related")) %>%
...
# DLBCL -------------------------------------------------------------------
decon_patho_gather_dlbcl <- read.csv(file.path(vis_decon_path, "vis_dlbcl_decon_long.csv"))
...
d_stroma_vessel_related <- c("Stroma",
                             "Vessels",
                             "Epithelium")

dlbcl_patho <- c(d_tumor_related, d_lymphocytes_related, d_macrophage_related, d_stroma_vessel_related)
facet_name <- c("Tumor_related", "Stroma_Vessel_related", "Lymphocytes_related", "Macrophage_related")
...
decon_patho_gather_dlbcl_subset <- decon_patho_gather_dlbcl %>%
  ...
  mutate(Region = case_when(Region %in% d_tumor_related ~ "Tumor_related",
                            ...
                            Region %in% d_stroma_vessel_related ~ "Stroma_Vessel_related")) %>%
  mutate(Region = factor(Region, levels = facet_name))


decon_patho_gather_dlbcl_subset <- rbind(decon_patho_gather_dlbcl_subset, data.frame(Barcode = rep(NA, 4),
                                                                       Region = facet_name,
                                                                       CellType = rep("Granulocyte", 4),
                                                                       Fraction = rep(NA, 4), 
                                                                       Section = rep(NA, 4)))

decon_patho_gather_dlbcl_subset$CellType <- factor(decon_patho_gather_dlbcl_subset$CellType)
...
# plot_title = "Vis_breast_lung_decon_sbs_violin_fill_color.pdf"
pdf(file = paste0("/work/PRTNR/CHUV/DIR/rgottar1/spatial/Owkin_Pilot_Results/Manuscript_Figures/Fig2c/", plot_title),
    width = 7,
    height = 4.4)
print(p_vis_decon)
dev.off()
