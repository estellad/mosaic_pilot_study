...
results <- results %>% filter(cluster == "1_5_7_9_TME")
...
p_B <- ggplot(data = enrichment_results_plt_df, aes(x=Term, y=Odds.Ratio, fill=-log10(P.value))) +
...
plot_title = "Vis_B3_2_DE_Pathway_1_5_7_9_TME.pdf"
...
results <- results %>% filter(cluster == "4_14_TME")
gene_list <- results$gene # [1:100]
...
# Plot barplot of adjusted p-values
p_A <- ggplot(data = enrichment_results_plt_df, aes(x=Term, y=Odds.Ratio, fill=-log10(P.value))) +
  geom_bar(stat="identity") + 
  ggtitle("Enrichment Analysis Results") + 
  labs(fill = expression("-log"[10]*"p-value")) + 
  xlab("Enriched Terms") + 
...
plot_title = "Vis_B3_2_DE_Pathway_4_14_TME.pdf"
pdf(file = file.path(figpath, plot_title),
    width = 7,
    height = 5.5)
print(p_A)
