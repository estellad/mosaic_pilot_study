...
for(disease in disease_list){
  ...
  p_indication <- p1 + theme(plot.margin = unit(c(0.3, 0.15, 1, 0), "cm")) | 
    p2 + theme(plot.margin = unit(c(0.3, 0.5, 1, 0.15), "cm")) | 
    p3 + theme(plot.margin = unit(c(0, 0.1, 0, 0.5), "cm")) | 
    p4 + theme(plot.margin = unit(c(0, 0, 0, 0.1), "cm")) 
  
  assign(paste0("p_", disease), p_indication)
}
...
figpath <- "/work/PRTNR/CHUV/DIR/rgottar1/spatial/Owkin_Pilot_Results/Manuscript_Figures_Final/SuppFig/Geo_TSNE_Vis_UMAP_Tcells_Macro"
pdf(file.path(figpath, "Geo_Vis_TSNE_UMAP_Tcells_Macro.pdf"), width = 22, height = 15)
print(p_save)
dev.off()
