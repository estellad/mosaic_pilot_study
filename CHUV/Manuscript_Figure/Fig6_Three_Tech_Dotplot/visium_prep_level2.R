...
decon_result_path <- "/work/PRTNR/CHUV/DIR/rgottar1/spatial/Owkin_Pilot_Results/Visium_Decon/Manuscript_Figures/Fig2c"
dlbcl_decon <- read.csv(file.path(decon_result_path, "vis_dlbcl_decon_long.csv"))

dlbcl_decon_ <- dlbcl_decon %>%
  pivot_wider(names_from = CellType, values_from = Fraction) %>%
  mutate(Stroma = Fibro_muscle + Vessel) %>%
  select(-c(Fibro_muscle, Vessel)) %>%
  pivot_longer(cols = c(Tumor, Stroma, Myeloid, B, T_NK, Epithelia), names_to = "CellType", values_to = "Fraction") %>%
...
# B cells Epithelia   Myeloid    Stroma      T/NK     Tu_D1     Tu_D2     Tu_D3     Tu_D4     Tu_D5     Tu_D6 
#   18588     18588     18588     18588     18588      4704      4114      4951      1436      1733      1650 
...
# Tu_D6     Tu_D5     Tu_D4     Tu_D3     Tu_D2     Tu_D1      T/NK   Myeloid   B cells    Stroma Epithelia 
# 497       269       181      2265      3721      3804      1446      1096        52      3373      1884 


dlbcl_decon_max <- dlbcl_decon_max[dlbcl_decon_max$Barcode2 %in% colnames(vis), ] # n = 18580
...
plot_title = "Vis_ridge_decon_max.pdf"
...
plot_title = "Vis_ridge_decon_max_50.pdf"
pdf(file = file.path(figpath_ridge, plot_title),
    width = 12.5,
    height = 8)
print(p)
...
saveRDS(vis_small, file.path(vispath, "Dlbcl-merge-SCTpostSpotClean_small_fig6e_level2.rds"))
