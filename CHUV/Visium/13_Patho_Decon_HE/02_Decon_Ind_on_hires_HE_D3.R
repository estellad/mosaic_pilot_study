...
# -------------------------------------------------------------------------
data <- data %>%
  select(Tu_D3_FAM3C, Tu_D3_dividing, x, y) 
...
p <- p0 + 
  ggforce::geom_circle(
    ...
    aes(
      ...
      fill = Tu_D3_dividing, # Tu_D3_FAM3C,
      colour = Tu_D3_dividing, # Tu_D3_FAM3C
    ),
    inherit.aes = FALSE
  ) +
  labs(x = NULL, y = NULL) + 
...
pdf(file = file.path(paste0(fig_path, "/Fig4b_deconHE"), 
                     # paste0(sample, "_decon_HE_Tu_D3_FAM3C.pdf")),
                     paste0(sample, "_decon_HE_Tu_D3_dividing.pdf")),
    width = plot_width,
    height = plot_height)
print(p)
dev.off()
