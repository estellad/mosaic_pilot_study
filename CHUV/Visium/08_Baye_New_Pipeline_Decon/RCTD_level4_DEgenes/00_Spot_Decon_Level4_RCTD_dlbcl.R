...
# Subset to DE genes ------------------------------------------------------
sigmat_path <- "/work/PRTNR/CHUV/DIR/rgottar1/spatial/Owkin_Pilot_Results/Visium_Decon/Level4/DEgenes_sigmat"

sigmat <- read.csv(file.path(sigmat_path, paste0(str_replace(save_names[i], "LBCL_", ""), "_signature.csv")))
chrom <- chrom[rownames(chrom) %in% sigmat$X, ]
...
# RCTD --------------------------------------------------------------------
myRCTD <- create.RCTD(puck, ref, max_cores = 2, CELL_MIN_INSTANCE = 5)
myRCTD <- run.RCTD(myRCTD, doublet_mode = 'full')

norm_weights <- normalize_weights(myRCTD@results$weights)
RCTD_results <- data.frame(as(norm_weights, "matrix"))

result_path <- "/work/PRTNR/CHUV/DIR/rgottar1/spatial/Owkin_Pilot_Results/Visium_Decon/Level4/RCTD_DEgenes"
