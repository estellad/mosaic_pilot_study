...
#                                 Subspot                                  #
############################################################################
# Mapped subspot and AOI IDs (spot)
mapped_geo_path <- "/work/PRTNR/CHUV/DIR/rgottar1/spatial/Owkin_Pilot_Results/Registration/GeoMx_mapped_subspot_LW"
geo_samples <- c("B1_1", "B1_3", "B2_1", "B3_1", "B4_1", "L1_1", "L2_1", "L3_3", "L4_1")
vis_samples <- c("B1_2", "B1_4", "B2_2", "B3_2", "B4_2", "L1_2", "L2_2", "L3_2", "L4_2")
...
for(i in 1:length(geo_samples)){
  ...
  mapped_i <- mapped_i%>% 
    select(barcode, Section_ID, Sample_ID, Cell_fraction, Region) %>%
    dplyr::rename(cell_fraction = Cell_fraction) %>%
    mutate(Section_ID_Vis = vis_samples[i])
  mapped_i$barcode2 <- mapped_i$barcode
  mapped_i$barcode <- rownames(mapped_i)
  rownames(mapped_i) <- NULL
  
  mapped_all <- rbind(mapped_all, mapped_i)
}

mapped_all <- mapped_all %>% # 78 Visium subspots from B1_2 patho exclude
  filter(!is.na(Region)) 

write.csv(mapped_all, 
          file.path("/work/PRTNR/CHUV/DIR/rgottar1/spatial/Owkin_Pilot_Results/Registration", "subspot_mapped_cf_region.csv"), row.names = FALSE)
...
#                     paste0(geo_samples[i], "_mapped_LW.csv"))) # column Sample_ID to be filtered out
